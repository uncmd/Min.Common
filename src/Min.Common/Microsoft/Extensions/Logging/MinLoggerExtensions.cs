using Min.Common.ExceptionHandling;
using Min.Common.Logging;
using System.Text;

namespace Microsoft.Extensions.Logging;

public static class MinLoggerExtensions
{
    ...
    private static void LogSelfLogging(ILogger logger, Exception exception)
    {
        ...
        if (exception is IExceptionWithSelfLogging selfLogging)
        {
            loggingExceptions.Add(selfLogging);
        }
        else if (exception is AggregateException aggException && exception.InnerException != null)
        {
            if (aggException.InnerException is IExceptionWithSelfLogging withSelfLogging)
            {
                loggingExceptions.Add(withSelfLogging);
            }

            foreach (var innerException in aggException.InnerExceptions)
            {
                if (innerException is IExceptionWithSelfLogging innerSelfLogging)
                {
                    loggingExceptions.AddIfNotContains(innerSelfLogging);
                }
            }
        }
